...
try{
        ...
        string jsonData=File.ReadAllText(MyFilePath);
          ...
          //Basic Data
            ...
            DataBoxJobDetails jobDetails = new DataBoxJobDetails(contactDetails, shippingAddress, destinationAccountDetails);

            Sku sku = new Sku();
           // sku.Name = SkuName.DataBoxDisk;
            sku.Name = SkuName.DataBox;
            JobResource newJobResource = new JobResource();
            newJobResource.Location = location.Replace(" ","").ToLower();
            ...
            try
            {
                var newJobResource1 = JobsOperationsExtensions.Get(dataBoxManagementClient.Jobs, resourceGroupName, jobName, expand);
                jobExist = true;
            }
            catch (Exception ex)
            {
            ...
            // Validate shipping address
             AddressValidationOutput addressValidateResult =
                ServiceOperationsExtensions.ValidateAddressMethod(
                    dataBoxManagementClient.Service, newJobResource.Location, shippingAddress,SkuName.DataBox);
